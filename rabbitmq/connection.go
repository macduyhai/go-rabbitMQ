	...
	go func() {
		for {
			...
			for {
				...
				logger.LogWarning("Checking connect...")
				conn, err := amqp.Dial(url)
				if err != nil {
					logger.LogError("reconnect failed, err:" + err.Error())
					continue
				}
				...
				logger.LogInfor("reconnect success")
				break
			}
		}
	}()
...
func Dial(url string) (*Connection, error) {
	...
	go func() {
		for {
			...
			logger.LogError("connection closed, reason:" + reason.Error())
...
func (c *Connection) Channel() (*Channel, error) {
	...
	go func() {
		for {
			...
			logger.LogWarning("channel closed, reason:" + reason.Error())
			...
			for {
				...
				if c.Connection.IsClosed() {
					logger.LogError("channel recreate failed, connection still closed")
					continue
				}

				ch, err := c.Connection.Channel()
				if err != nil {
					logger.LogError("channel recreate failed, err:" + err.Error())
					continue
				}
				...
				if channel.qos.applied {
					...
					if err != nil {
						ch.Close()
						logger.LogWarning(fmt.Sprintf("channel recreate failed, unable to restore qos settings, err: %s (prefetch_count: %d; prefetch_size: %d; global: %t)", err, channel.qos.prefetchCount, channel.qos.prefetchSize, channel.qos.global))
						continue
					}

					logger.LogWarning(fmt.Sprintf("qos restored (prefetch_count: %d; prefetch_size: %d; global: %t)", channel.qos.prefetchCount, channel.qos.prefetchSize, channel.qos.global))
				}
				...
				for _, q := range channel.autoDeletedQueues {
					...
					if err != nil {
						logger.LogWarning(fmt.Sprintf("channel recreate failed, unable to restore auto deleted queue, err: %v (name: %s; durable: %t; auto_delete: %t; exclusive: %t; no_wait: %t; passive: %t)", err, q.name, q.durable, q.autoDelete, q.exclusive, q.noWait, q.passive))
						queuesRestored = false
						break
					}

					logger.LogWarning(fmt.Sprintf("queue restored (name: %s; durable: %t; auto_delete: %t; exclusive: %t; no_wait: %t; passive: %t)", q.name, q.durable, q.autoDelete, q.exclusive, q.noWait, q.passive))
				}
				...
				for _, e := range channel.autoDeletedExchanges {
					...
					if err != nil {
						logger.LogError(fmt.Sprintf("channel recreate failed, unable to restore auto deleted exchange, err: %v (name: %s; kind: %s; durable: %t; auto_delete: %t; internal: %t; no_wait: %t)", err, e.name, e.kind, e.durable, e.autoDelete, e.internal, e.noWait))
						exchangesRestored = false
						break
					}

					logger.LogWarning(fmt.Sprintf("exchange restored (name: %s; kind: %s; durable: %t; auto_delete: %t; internal: %t; no_wait: %t)", e.name, e.kind, e.durable, e.autoDelete, e.internal, e.noWait))
				}
				...
				for _, b := range channel.autoDeletedQueueBindings {
					if err := ch.QueueBind(b.queueName, b.key, b.exchangeName, b.noWait, b.args); err != nil {
						logger.LogError("channel recreate failed, unable to restore auto deleted queue or exchange binding, err: %v")
				...
				logger.LogInfor("channel recreate success")
